      ...
      {projects.map((project) => (
        <div key={project.id} className="bg-card border border-border rounded-lg p-6">
          <div className="flex items-center justify-between mb-3">
            <span className={`category-${project.category} px-2 py-1 rounded text-xs font-medium`}>
              {project.category.replace("-", " ")}
            </span>
            <div className="flex items-center gap-2">
              <a href={project.github} className="text-muted-foreground" target="_blank" rel="noopener noreferrer">
                <Github size={16} />
              </a>
              <a href={project.demo} className="text-muted-foreground" target="_blank" rel="noopener noreferrer">
